...
hairSaloon <- simmer("hairSaloon")%>%
  ...
  add_resource("cutStation",capacity=8,queue_size=Inf)%>%
...
arrivalData1 <- get_mon_arrivals(mm, ongoing = TRUE)
resourceData1 <- get_mon_resources(mm)
attributeData1 <- get_mon_attributes(mm)
arrivalData1all <- get_mon_arrivals(mm, ongoing =TRUE)

#-------------------------------- ממוצע וסטיית תקן ל3 מדדים --------------------------------

#----------------------- מדד זמן ממוצע בתור לאדם ביום ----------------------

arrivalData1withAvgQueue1<-mutate(arrivalData1, waiting = (end_time - start_time - activity_time))

avgWaitingByreplication1 <- sqldf(
  "select replication,avg(waiting)as AW
  from arrivalData1withAvgQueue1
  where finished= true
  group by replication
  ")
...
avgWaiting1 <- sqldf(
  "select avg(AW)
  from avgWaitingByreplication1
  ")
print(avgWaiting1)
print(sd(avgWaitingByreplication1$AW))

# one-way t-test 
test<- t.test(x= avgWaitingByreplication1$AW,y=, alternative="two.sided",conf.level=0.7)
print(test)

#----------------------- אורך תור ממוצע לעמדת תספורת ----------------------

avgQueueByreplication1 <- sqldf(
...
avgQueue1 <- sqldf(
  "select avg(AQ)
  from avgQueueByreplication1
  ")
print(avgQueue1)
print(sd(avgQueueByreplication1$AQ))


#------------------------------ אחוז מסיימים יומי ---------------------------

howManyArrive1 <- sqldf("select count(DISTINCT name) as CA
                               from arrivalData1all
                              group by replication")
paste("total arival",howManyArrive1)


howManyFinish1 <- sqldf("select COUNT(DISTINCT name) as CA
                               from arrivalData1all
                               where finished=TRUE
                       group by replication")
paste("total finish",howManyFinish1)

finishPrecent1<-howManyFinish1/howManyArrive1

avgFinish1<- sqldf("select avg(CA)
                  from finishPrecent1
                    ")
print(avgFinish1)
sd(finishPrecent1$CA)
